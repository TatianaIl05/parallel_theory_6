...
Config parse_args(int argc, char** argv) {
    ...
    desc.add_options()
        ...
        ("check-iter,c", po::value<int>(&cfg.check_iter)->default_value(100), "Проверка ошибки каждые N итераций")
...
int main(int argc, char** argv) {
    ...
    std::cout << "Проверка ошибки каждые: " << cfg.check_iter << " итераций" << std::endl;
    ...
    #pragma acc data copy(cur[0:size], nxt[0:size])
    {
        while (iter < cfg.max_iter) {
            bool need_check = ((iter + 1) % cfg.check_iter == 0);
            
            if (need_check) {
                error = 0.0;
                
                #pragma acc parallel loop collapse(2) vector_length(256) reduction(max:error) present(cur[0:size], nxt[0:size])
                ...
                #pragma acc parallel loop collapse(2) vector_length(256) present(cur[0:size], nxt[0:size])
                for (int i = 1; i < cfg.ny - 1; ++i) {
                    for (int j = 1; j < cfg.nx - 1; ++j) {
                        double val = 0.25 * (cur[(i + 1) * N + j] +
                                             cur[(i - 1) * N + j] +
            ...
            if (need_check && error < cfg.eps) break;
        }
    }
